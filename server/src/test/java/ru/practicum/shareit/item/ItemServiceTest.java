...
public class ItemServiceTest {
    ...
    void testGetAllItems() {
        Mockito.when(itemRepository.findAllByOwnerIdOrderById(anyInt())).thenAnswer(invocationOnMock -> {
            int itemId = invocationOnMock.getArgument(0, Integer.class);
            if (itemId == item.getId()) {
                return List.of(item);
            } else {
        ...
        Mockito.verify(itemRepository, Mockito.times(1)).findAllByOwnerIdOrderById(user.getId());
        Mockito.verify(commentRepository, Mockito.times(1)).findAllByItemId(item.getId());
